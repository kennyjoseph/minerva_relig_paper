...
sfExport("get_network_data")
sfExport("get_network_stats")
sfExport("SUSP_FN")
...
#REPLY NETWORK
#MENTIONS NETWORK
network_files <- Sys.glob(paste0(NETWORK_DIR,"*"))
basic_nets <- parLapply(sfGetCluster(),network_files, function(network_file){
...
get_resampled <- function(t, eta_sample_set, sigma, data, 
  ...
  print(paste(sum(is.na(weights)),sum(is.nan(weights)), sum(weights),sum(weights/sum(weights))))
...
for(type_var in c("TWITTER","NEWS")){
  for(category_var in ALL_CATEGORIES){
    ...
    save(per_cat_eta_samples_list, file=file.path("result_data",paste0(type,"_",category,"_eta_samples.rdata")))
